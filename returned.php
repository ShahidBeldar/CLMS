...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add') {
        ...
        $issue_date = date("Y-m-d"); // Corrected to use Issue_Date
        $return_date = date("Y-m-d"); // Return date is the same as the issue date for a new return record

        $stmt = $conn->prepare("INSERT INTO Report (Student_Id, Book_Id, Issue_Date, Due_Date, Return_Date, Status) VALUES (?, ?, ?, ?, ?, 'Returned')");
        $stmt->bind_param("sssss", $student_id, $book_id, $issue_date, $due_date, $return_date);
        $stmt->execute();
        $stmt->close();
        ...
        $student_id = $_POST['Student_Id'] ?? '';
        $book_id = $_POST['Book_Id'] ?? '';
        $status = $_POST['Status'] ?? 'Returned';

        // Check if the status is being changed to 'Not Returned'
        if ($status === 'Not Returned') {
            ...
            // Keep Return_Date as NULL if status is 'Returned'
            $stmt = $conn->prepare("UPDATE Report SET Student_Id=?, Book_Id=?, Status=? WHERE Reg_no=?");
            $stmt->bind_param("ssss", $student_id, $book_id, $status, $reg_no);
        }
        $stmt->execute();
        $stmt->close();
...
$returned_books = [];
$sql = "SELECT r.Reg_no, r.Student_Id, r.Book_Id, r.Issue_Date, r.Due_Date, r.Return_Date, r.Status,
               s.Name AS Student_Name, b.Name AS Book_Name
        FROM Report r
        JOIN Student s ON r.Student_Id = s.Student_Id
        JOIN Book b ON r.Book_Id = b.Book_Id
        WHERE r.Status = 'Returned'
        ORDER BY r.Return_Date DESC"; // Corrected to use Return_Date in ORDER BY clause
...
<div class="main">
    <div class="top-actions">
        <div>
            ...
            <p class="welcome">Browse and manage records for books that have been returned.</p>
        </div>
        <div class="left-actions">
            <button class="btn add" id="addBtn">Add Return</button>
        </div>
    </div>
    ...
    <table id="returnedTable">
        ...
        <tbody>
            <?php foreach ($returned_books as $b): ?>
                <tr data-id="<?= htmlspecialchars($b['Reg_no']) ?>">
                    ...
                    <td class="c-return-date"><?= htmlspecialchars($b['Return_Date']) ?></td>
...
<script>
    // Handles inline editing and form submissions for returned books
    document.getElementById('addBtn').addEventListener('click', function() {
        if (document.querySelector('#returnedTable tbody tr.adding')) return;
        const tbody = document.querySelector('#returnedTable tbody');
